...
namespace UnityEngine.Localization
{
    class AssetDatabasePreloadOperation : AsyncOperationBase<LocalizedAssetDatabase>
    {
        ...
        void TableLoaded(IAsyncOperation<LocalizedAssetTable> asyncOperation)
        {
            ...
            Debug.AssertFormat(!tables.ContainsKey(asyncOperation.Result.TableName),
                "A table with the same key `{0}` already exists for this type `{1}`. Something went wrong during preloading.",
                asyncOperation.Result.TableName, asyncOperation.Result.SupportedAssetType);
        ...
        void PreloadTablesCompleted(IAsyncOperation<IList<LocalizedAssetTable>> asyncOperation)
        {
            ...
            {
                if (table is IPreloadRequired preloadTable && !preloadTable.PreloadOperation.IsDone)
                {
